...
public static class UiKit
{
	...
	public static void DrawHelp(CanvasItem c, Font f)
	{
		...
		Sec("大地图(行军)");
		K("WASD / 点击", "行军(官道快,林地慢,河不可渡——找渡滩)");
		K("C", "扎营入中军帐(扎营才可用沙盘与瞭望台)");
		K("Esc", "暂停菜单");
		y += 10;

		Sec("中军帐 · 营区(操纵将军走动)");
		K("WASD", "将军行走(战事在你走动时照常推进!)");
		K("E", "交互:近沙盘=推演 · 近瞭望台=登台 · 台上=下台");
		K("F1", "低难度开关:沙盘直接叠加瞭望所见");
		K("Esc", "台上=下台 · 其余=暂停菜单(拔营走南辕门)");
		y += 10;

		Sec("沙盘(战斗指挥——你只看得见『送到手上的信息』)");
		K("空格 / + -", "暂停 / 调速          N=单步");
		K("左键 / 右键", "选部 / 令骑传令行军(Shift=疾进)");
		K("1 2 3 4", "姿态令:进攻 / 据守 / 等待 / 游走(风筝)");
		K("中键 / R / B", "塘骑侦察 / 探问该部 / 军书具报行营");
		K("Ctrl+左键", "插信息旗(Ctrl+右键拔旗)");
		K("Tab / Esc", "真实战场对照(开发用) / 回中军帐");
